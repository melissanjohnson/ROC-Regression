...
gammak <- list(c(0,0, 0, 0, 0),
               c(0,0, 0,0, 0.7416143),
               c(0,0,0, 0.7416143, 0.7416143),
               c(0,0, 0.7416143, 0.7416143, 0.7416143),
               c(0,0.7416143, 0.7416143, 0.7416143, 0.7416143))
...
for(i in 1:ntime){
  ...
  data_frame(x = seq(0.01, 0.99, by = 0.025)) %>% 
...
SE2 <- list(Lehmann = matrix(NA, nrow = nsim, ncol = ntime),
           Beta = matrix(NA, nrow = nsim, ncol = ntime))
for(i in 1:ntime){
  SE2$Lehmann[,i] <- (Youden$Lehmann[,i] - trueYouden[[ss]][i])^2
  SE2$Beta[,i] <- (Youden$Beta[,i] - trueYouden[[ss]][i])^2
}
